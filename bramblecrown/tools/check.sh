...
fail=0
mkdir -p build/check-logs

echo "== import"
out=$(timeout 400 "$GODOT" --headless --editor --path . --quit 2>&1)
status=$?
printf '%s\n' "$out" > build/check-logs/import.log
if [ "$status" -ne 0 ]; then echo "Import exit: $status"; fail=1; fi
...
for s in title combat map; do
	...
	status=$?
	printf '%s\n' "$out" > "build/check-logs/scene-$s.log"
	if [ "$status" -ne 0 ]; then echo "Scene $s exit: $status"; fail=1; fi
	if echo "$out" | grep -E "SCRIPT ERROR|Parse Error|^ERROR"; then echo "   in scene $s"; fail=1; fi
done

echo "== rule tests"
out=$(timeout 300 "$GODOT" --headless --path . -s res://tests/test_runner.gd 2>&1)
status=$?
printf '%s\n' "$out" > build/check-logs/rules.log
if [ "$status" -ne 0 ]; then echo "Rules exit: $status"; fail=1; fi
